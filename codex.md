...
- v0.31 前三步没有新增持久化字段，存档 schema 不升级。
...
- Day 30 与 Day 60 原确定性指纹继续成功，无需更新基准；
...
7. 草棚、储物棚和农田还不是寻路障碍。
8. 当前正式响应仍集中在食物、饮水、木材和储存材料；种子、播种、收获、土壤与劳动积压尚未接入承诺评分。
9. `commitmentBlocked` 已由评分器返回，但尚未进入最终任务解释或观察器 UI。
10. 自动 Chromium 不能替代真实 iPhone 安全区与浏览器外壳回归。

## v0.31 后续步骤

1. 补齐种子、播种、收获、储存建设、土壤与劳动积压的正式响应映射，并确保对应行动确实能进入合法候选集合。
2. 保存最终选中行动的轻量解释，显示贡献效果、劳动力缺口和不可执行承诺。
3. 为新增农业与积压响应建立专用行为场景，再执行 Day 30、Day 60、多批次和 Day 120 验证。
4. 完成 v0.31 发布收口后再更新 `version.json`、README 和存档应用版本。
...
- `v0.31`（开发中）：第一步建立候选效果目录；第二步建立组合级承诺劳动力规划；第三步将效果和劳动力缺口接入评分并在满员后停止加分。

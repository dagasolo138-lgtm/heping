...
test('已完成承诺不加分，多项真实响应承诺合计仍封顶 18', () => {
  const score = scoreCommitmentUtility({
    ...
    commitments: [
      ...
      { id: 'wood-normal', type: 'restore-wood-reserve', state: 'active', priority: 100, progress: 0.5 },
      { id: 'wood-urgent', type: 'restore-wood-reserve', state: 'active', priority: 100, progress: 0 },
    ],
  });

  assert.equal(score.score, 18);
  assert.deepEqual(score.matches.map((entry) => entry.id), ['wood-urgent', 'wood-normal']);
});

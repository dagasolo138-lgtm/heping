...
import { getChunkAt, getFeaturesAt, getTile, isWalkable } from './mapQueries.js';
...
function hydrateMap(snapshot) {
  ...
  if (snapshot.terrain?.length !== expected || snapshot.elevation?.length !== expected) {
    throw new Error('地图格子数量与尺寸不一致。');
  }
  return {
    ...clone(snapshot),
    terrain: Uint8Array.from(snapshot.terrain),
    elevation: Uint8Array.from(snapshot.elevation),
  };
}
...
export function createMapSystem({ eventBus, gameTime }) {
  ...
  function exportState() {
    return map ? serializeMap(map) : null;
  }

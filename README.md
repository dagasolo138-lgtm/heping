...
- 动态库存参数与劳动成本系数仍是原型平衡值，需要第 60 日长期回归校准。
- 候选评分使用直线成本预估；任务真正分配后才使用 A* 路线生成精确劳动快照。
- 工具与工具耐久尚未进入劳动成本，当前技能只降低额外能耗。
...
v0.27 第三步进入 **统一资源流水**：采集、搬运、施工、进食、添柴与腐败全部记录计划量、实际量、来源、去向和失败原因。完整顺序见 [ROADMAP.md](./ROADMAP.md)。

...
人物背包、营地库存和工具耐久的变化现在进入同一套可追溯账本。每笔记录包含：
...
同一世界 tick 内，明确属于同一任务或搬运链的账户增减会自动配对：
...
独立发生的生产与消费不会互相抵消。未配对的资源增加会归入河流、地图物件、农田或其他生产源；未配对的减少会归入进食、饮水、篝火、施工、腐败或其他消费去向。工具磨损和修理以 `durability` 单位进入同一流水。

流水最多保留最近 5,000 笔，并随世界存档保存。旧存档没有 `resourceFlow` 字段时从空账本开始；失败读档会恢复读取前的流水检查点。
...
`verify()` 会检查重复记录、非法数量、无效流向、人物与营地负库存，以及工具耐久越界。营地面板显示当日生产、消耗、施工、腐败和转移汇总。
...
流水模块作为旁路事实账本，不改变原世界结算顺序，因此原确定性内核基线保持不变。
...
- 修理与替换 API 已进入耐久流水，但尚未形成需要材料与劳动时间的正式维修任务。
- 石镐已进入工具目录，采石行动和石材资源尚未实现。
- 流水当前按账户实际增减记账；“计划量、失败原因和完整工序批次”仍需在后续日报阶段继续细化。
- 当前每日汇总只展示收支类别，尚未生成瓶颈、积压、劳动占比和库存期初期末对账。
- 草棚、储物棚和农田尚未成为寻路障碍。
- 真实 iPhone 的安全区、浏览器地址栏变化和极小屏可读性仍需人工回归。

## 下一阶段

v0.27 第五步进入 **每日经济摘要与第 60 日回归**：从资源流水计算日收支、期初期末库存、劳动分配、瓶颈与异常，并验证不同倍速在第 60 日得到相同结果。完整顺序见 [ROADMAP.md](./ROADMAP.md)。
